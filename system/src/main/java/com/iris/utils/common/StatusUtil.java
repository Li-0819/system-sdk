 ...
 * @description: 通用状态码
 * @author: RubyWong
 * @date: Created in 2019-12-07 14:31
 * @copyright: Copyright (c) 2018
 * @version: V1.0
 * @modified: RubyWong
 */
@Getter
public enum StatusUtil implements IStatusUtil {
    /**
     * 操作成功！
     */
    SUCCESS(200, "操作成功！"),
    ...
    StatusUtil(Integer code, String message) {
        this.code = code;
        this.message = message;
    }

    public static StatusUtil fromCode(Integer code) {
        StatusUtil[] statuses = StatusUtil.values();
        for (StatusUtil status : statuses) {
            if (status.getCode()
                    .equals(code)) {
                return status;
            }
